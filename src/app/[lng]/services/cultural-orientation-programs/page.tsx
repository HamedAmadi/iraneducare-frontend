...
const CivicEducation = async ({ params: { lng } }: { params: { lng: string; } }) => {
    ...
    const { t: tCOP } = await useTranslation(lng, 'cultural-orientation-programs');
            ...
            <div className='relative w-full h-fit -z-10 bg-center bg-no-repeat bg-cover bg-[url("../../../public/images/world-map.png")]'>
                ...
                <div className="xs:container px-4">
                    <div className="pt-[clamp(6rem,16vw,10rem)] pb-[clamp(3rem,10vw,7rem)] text-center text-text">
                        <h1 className="text-[clamp(1.75rem,4vw,3rem)] font-BYekan">{tCOP('culturalOrientation.title')}</h1>
                    </div>
                </div>
            </div>
                ...
                <div className="grid grid-cols-12 md:gap-8">
                    ...
                    <div className="col-span-12 lg:col-span-8">
                        ...
                        <section className="mb-8">
                            <h1 className="text-[clamp(1.25rem,4vw,1.875rem)] font-bold text-text mb-6">{tCOP('culturalOrientation.subtitle')}</h1>
                            <p className="md:text-lg text-[#335371]">{tCOP('culturalOrientation.description')}</p>
                        </section>

                        {/* چرا آشنایی با فرهنگ ایران مهم است؟ */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tCOP('whyCulturalOrientation.title')}</h2>
                            <ul className="ps-8 list-disc md:text-lg text-[#335371] space-y-4">
                                <li><strong>{tCOP('whyCulturalOrientation.point1-title')}</strong> - {tCOP('whyCulturalOrientation.point1-desc')}</li>
                                <li><strong>{tCOP('whyCulturalOrientation.point2-title')}</strong> - {tCOP('whyCulturalOrientation.point2-desc')}</li>
                                <li><strong>{tCOP('whyCulturalOrientation.point3-title')}</strong> - {tCOP('whyCulturalOrientation.point3-desc')}</li>
                                <li><strong>{tCOP('whyCulturalOrientation.point4-title')}</strong> - {tCOP('whyCulturalOrientation.point4-desc')}</li>
                            </ul>
                        </section>

                        {/* موضوعات دوره‌های آشنایی با فرهنگ ایران */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tCOP('programCoverage.title')}</h2>
                            <ul className="ps-8 list-disc md:text-lg text-[#335371] space-y-4">
                                <li><strong>{tCOP('programCoverage.point1-title')}</strong> - {tCOP('programCoverage.point1-desc')}</li>
                                <li><strong>{tCOP('programCoverage.point2-title')}</strong> - {tCOP('programCoverage.point2-desc')}</li>
                                <li><strong>{tCOP('programCoverage.point3-title')}</strong> - {tCOP('programCoverage.point3-desc')}</li>
                                <li><strong>{tCOP('programCoverage.point4-title')}</strong> - {tCOP('programCoverage.point4-desc')}</li>
                                <li><strong>{tCOP('programCoverage.point5-title')}</strong> - {tCOP('programCoverage.point5-desc')}</li>
                            </ul>
                        </section>

                        {/* مراحل شرکت در برنامه */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tCOP('participationSteps.title')}</h2>
                            <ul className="md:text-lg text-[#335371] space-y-4">
                                <li><strong>{tCOP('participationSteps.step1-title')}:</strong> {tCOP('participationSteps.step1-desc')}</li>
                                <li><strong>{tCOP('participationSteps.step2-title')}:</strong> {tCOP('participationSteps.step2-desc')}</li>
                                <li><strong>{tCOP('participationSteps.step3-title')}:</strong> {tCOP('participationSteps.step3-desc')}</li>
                                {/* <li><strong>{tCOP('participationSteps.step4-title')}:</strong> {tCOP('participationSteps.step4-desc')}</li> */}
                            </ul>
                        </section>

                        {/* چرا برنامه‌های ما؟ */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tCOP('whyJoin.title')}</h2>
                            <ul className="ps-8 list-disc md:text-lg text-[#335371] space-y-1">
                                <li><strong>{tCOP('whyJoin.point1')}</strong></li>
                                <li><strong>{tCOP('whyJoin.point2')}</strong></li>
                                <li><strong>{tCOP('whyJoin.point3')}</strong></li>
                                <li><strong>{tCOP('whyJoin.point4')}</strong></li>
                            </ul>
                        </section>

                        {/* سوالات متداول */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tCOP('faq.title')}</h2>
                            <div className="mb-2">
                                <h3 className="font-semibold text-lg md:text-xl text-[#335371] mb-1">{tCOP('faq.q1')}</h3>
                                <p className="md:text-lg text-[#335371]">{tCOP('faq.a1')}</p>
                            </div>
                            <div className="mb-2">
                                <h3 className="font-semibold text-lg md:text-xl text-[#335371] mb-1">{tCOP('faq.q2')}</h3>
                                <p className="md:text-lg text-[#335371]">{tCOP('faq.a2')}</p>
                            </div>
                            <div className="mb-2">
                                <h3 className="font-semibold text-lg md:text-xl text-[#335371] mb-1">{tCOP('faq.q3')}</h3>
                                <p className="md:text-lg text-[#335371]">{tCOP('faq.a3')}</p>
                            </div>
                            <div className="mb-2">
                                <h3 className="font-semibold text-lg md:text-xl text-[#335371] mb-1">{tCOP('faq.q4')}</h3>
                                <p className="md:text-lg text-[#335371]">{tCOP('faq.a4')}</p>
                            </div>
                        </section>

                        {/* بخش تماس */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tCOP('contact.title')}</h2>
                            <p className="md:text-lg text-[#335371]">{tCOP('contact.description')}</p>
                            {/* <div className="relative mt-12">
                                <a href="/contact" className="relative px-[30px] py-[20px] lg:md:text-lg rounded-full bg-first text-white hover:text-text hover:bg-second">
                                    {tCOP('contact.button')}
                                </a>
                            </div> */}
                        </section>
