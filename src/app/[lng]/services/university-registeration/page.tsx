...
const UniversityRegistration = async ({ params: { lng } }: { params: { lng: string } }) => {
    ...
    const { t: tUR } = await useTranslation(lng, 'university-registeration');
            ...
            <div className='relative w-full h-fit -z-10 bg-center bg-no-repeat bg-cover bg-[url("../../../public/images/world-map.png")]'>
                ...
                <div className="xs:container px-4">
                    <div className="pt-[clamp(6rem,16vw,10rem)] pb-[clamp(3rem,10vw,7rem)] text-center text-text">
                        <h1 className="text-[clamp(1.75rem,4vw,3rem)] mb-2 md:mb-5 font-BYekan">{tUR('title')}</h1>
                        <h2 className="text-[clamp(1.5rem,4vw,1.875rem)] font-BYekan">{tUR('sub-title')}</h2>
                    </div>
                </div>
            </div>
                ...
                <div className="grid grid-cols-12 md:gap-8">
                    ...
                    <div className="col-span-12 lg:col-span-8">
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tUR('enrollment.title')}</h2>
                            <p className="md:text-lg text-[#335371] mb-4">{tUR('enrollment.desc')}</p>
                            <ul className="md:text-lg text-[#335371] space-y-4">
                                <li className="mb-4">
                                    <h3 className="text-lg md:text-xl mb-2"><strong className="text-lg md:text-xl mb-8">{tUR('enrollment.step1.title')}</strong></h3>
                                    <ul className="ps-8 list-disc space-y-1">
                                        {/* <li>{tUR('enrollment.step1.point1')} <Link href={`/${lng}/universities`} className="text-first">({tUR('enrollment.step1.point1-link')})</Link></li> */}
                                        <li>{(tUR("enrollment.step1.point1", { returnObjects: true }) as any[]).map((part: any, index: number) =>
                                            typeof part === "string" ? (
                                                part
                                            ) : (
                                                <Link key={index} href={`/${lng}/universities`} className="text-blue-600">
                                                    {part.link}
                                                </Link>
                                            )
                                        )}</li>
                                        <li>{tUR('enrollment.step1.point2')}</li>
                                        <li>{tUR('enrollment.step1.point3')}</li>
                                    </ul>
                                </li>
                                <li className="mb-4">
                                    <h3 className="text-lg md:text-xl mb-2"><strong>{tUR('enrollment.step2.title')}</strong></h3>
                                    <ul className="ps-8 list-disc space-y-1">
                                        <li>{tUR('enrollment.step2.point1')}</li>
                                        <li>{tUR('enrollment.step2.point2')}</li>
                                        <li>{tUR('enrollment.step2.point3')}</li>
                                        <li>{tUR('enrollment.step2.point4')}</li>
                                        <li>{tUR('enrollment.step2.point5')}</li>
                                    </ul>
                                    <div className="my-4">
                                        <h3 className="font-semibold text-lg md:text-xl text-[#335371] mb-1">{tUR('enrollment.step2.translationHelp')}</h3>
                                        <p className="md:text-lg text-[#335371]">{tUR('enrollment.step2.translationService')}</p>
                                    </div>
                                </li>
                                <li className="mb-4">
                                    <h3 className="text-lg md:text-xl mb-2"><strong>{tUR('enrollment.step3.title')}</strong></h3>
                                    <p>{tUR('enrollment.step3.description')}</p>
                                </li>
                                <li className="mb-4">
                                    <h3 className="text-lg md:text-xl mb-2"><strong>{tUR('enrollment.step4.title')}</strong></h3>
                                    <ul className="ps-8 list-disc space-y-1 mb-2">
                                        <li>{tUR('enrollment.step4.point1')}</li>
                                        <li>{tUR('enrollment.step4.point2')}</li>
                                    </ul>
                                    <p className="md:text-lg text-[#335371]">
                                        {(tUR("enrollment.step4.description", { returnObjects: true }) as any[]).map((part: any, index: number) =>
                                            typeof part === "string" ? (
                                                part
                                            ) : (
                                                <Link key={index} href={`/${lng}/services/visa`} className="text-blue-600">
                                                    {part.link}
                                                </Link>
                                            )
                                        )}
                                ...
                                <li className="mb-4">
                                    <h3 className="text-lg md:text-xl mb-2"><strong>{tUR('enrollment.step5.title')}</strong></h3>
                                    <p>{tUR('enrollment.step5.description')}</p>
                                </li>
                            </ul>
                        </section>
                        ...
                        {/* بخش چرا ما؟ */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tUR('whyUs.title')}</h2>
                            <ul className="ps-8 list-disc md:text-lg text-[#335371] space-y-1">
                                <li><strong>{tUR('whyUs.point1-title')}</strong> {tUR('whyUs.point1-desc')}</li>
                                <li><strong>{tUR('whyUs.point2-title')}</strong> {tUR('whyUs.point2-desc')}</li>
                                <li><strong>{tUR('whyUs.point3-title')}</strong> {tUR('whyUs.point3-desc')}</li>
                                <li><strong>{tUR('whyUs.point4-title')}</strong> {tUR('whyUs.point4-desc')}</li>
                            </ul>
                        </section>

                        {/* بخش سوالات متداول */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tUR('faq.title')}</h2>
                            <div className="mb-2">
                                <h3 className="font-semibold text-lg md:text-xl text-[#335371] mb-1">{tUR('faq.q1')}</h3>
                                <p className="md:text-lg text-[#335371]">{tUR('faq.a1-title')}</p>
                                <ul className="ps-8 md:text-lg text-[#335371] list-disc space-y-1">
                                    <li>{tUR('faq.a1-item1')}</li>
                                    <li>{tUR('faq.a1-item2')}</li>
                                    {/* <li>{tUR('enrollment.step4.point2')}</li> */}
                                </ul>
                            </div>
                            <div className="mb-2">
                                <h3 className="font-semibold text-lg md:text-xl text-[#335371] mb-1">{tUR('faq.q2')}</h3>
                                <p className="md:text-lg text-[#335371]">{tUR('faq.a2')}</p>
                            </div>
                            <div className="mb-2">
                                <h3 className="font-semibold text-lg md:text-xl text-[#335371] mb-1">{tUR('faq.q3')}</h3>
                                <p className="md:text-lg text-[#335371]">{tUR('faq.a3')}</p>
                            </div>
                        </section>

                        {/* بخش تماس */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tUR('contact.title')}</h2>
                            <p className="md:text-lg text-[#335371]">{tUR('contact.description')}</p>
