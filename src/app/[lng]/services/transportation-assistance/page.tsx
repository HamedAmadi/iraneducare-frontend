...
const TransportationAssistance = async ({ params: { lng } }: { params: { lng: string; } }) => {

    if (languages.indexOf(lng) < 0) lng = fallbackLng;
    const { t: tTA } = await useTranslation(lng, 'transportation-assistance');
            ...
            <div className='relative w-full h-fit -z-10 bg-center bg-no-repeat bg-cover bg-[url("../../../public/images/world-map.png")]'>
                ...
                <div className="xs:container px-4">
                    <div className="pt-[clamp(6rem,16vw,10rem)] pb-[clamp(3rem,10vw,7rem)] text-center text-text">
                        <h1 className="text-[clamp(1.75rem,4vw,3rem)] font-BYekan">{tTA('title')}</h1>
                    </div>
                </div>
            </div>
                ...
                <div className="grid grid-cols-12 md:gap-8">
                    ...
                    <div className="col-span-12 lg:col-span-8">
                        ...
                        <section className="mb-8">
                            <h1 className="text-[clamp(1.25rem,4vw,1.875rem)] font-bold text-text mb-6">{tTA('sub-title')}</h1>
                            <p className="md:text-lg text-[#335371]">{tTA('introduction')}</p>
                        </section>

                        {/* خدمات النقل */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tTA('services.title')}</h2>
                            <ul className="ps-8 list-disc md:text-lg text-[#335371] space-y-4">
                                <li><strong>{tTA('services.service1-title')}</strong> - {tTA('services.service1-desc')}</li>
                                <li><strong>{tTA('services.service2-title')}</strong> - {tTA('services.service2-desc')}</li>
                                <li><strong>{tTA('services.service3-title')}</strong> - {tTA('services.service3-desc')}</li>
                                <li><strong>{tTA('services.service4-title')}</strong> - {tTA('services.service4-desc')}</li>
                                <li><strong>{tTA('services.service5-title')}</strong> - {tTA('services.service5-desc')}</li>
                            </ul>
                        </section>

                        {/* كيفية استخدام خدماتنا */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tTA('howToApply.title')}</h2>
                            <ul className="md:text-lg text-[#335371] space-y-4">
                                <li><strong>{tTA('howToApply.step1-title')}:</strong> {tTA('howToApply.step1-desc')}</li>
                                <li><strong>{tTA('howToApply.step2-title')}:</strong> {tTA('howToApply.step2-desc')}</li>
                                <li><strong>{tTA('howToApply.step3-title')}:</strong> {tTA('howToApply.step3-desc')}</li>
                            </ul>
                        </section>

                        {/* لماذا نحن؟ */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tTA('whyUs.title')}</h2>
                            <ul className="ps-8 list-disc md:text-lg text-[#335371] space-y-1">
                                <li><strong>{tTA('whyUs.point1-title')}</strong> {tTA('whyUs.point1-desc')}</li>
                                <li><strong>{tTA('whyUs.point2-title')}</strong> {tTA('whyUs.point2-desc')}</li>
                                <li><strong>{tTA('whyUs.point3-title')}</strong> {tTA('whyUs.point3-desc')}</li>
                                <li><strong>{tTA('whyUs.point4-title')}</strong> {tTA('whyUs.point4-desc')}</li>
                            </ul>
                        </section>

                        {/* الأسئلة الشائعة */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tTA('faq.title')}</h2>
                            <div className="mb-2">
                                <h3 className="font-semibold text-lg md:text-xl text-[#335371] mb-1">{tTA('faq.q1')}</h3>
                                <p className="md:text-lg text-[#335371]">{tTA('faq.a1')}</p>
                            </div>
                            <div className="mb-2">
                                <h3 className="font-semibold text-lg md:text-xl text-[#335371] mb-1">{tTA('faq.q2')}</h3>
                                <p className="md:text-lg text-[#335371]">{tTA('faq.a2')}</p>
                            </div>
                            <div className="mb-2">
                                <h3 className="font-semibold text-lg md:text-xl text-[#335371] mb-1">{tTA('faq.q3')}</h3>
                                <p className="md:text-lg text-[#335371]">{tTA('faq.a3')}</p>
                            </div>
                            <div className="mb-2">
                                <h3 className="font-semibold text-lg md:text-xl text-[#335371] mb-1">{tTA('faq.q4')}</h3>
                                <p className="md:text-lg text-[#335371]">{tTA('faq.a4')}</p>
                            </div>
                        </section>

                        {/* حجز الخدمة */}
                        <section className="mb-8">
                            <h2 className="text-[clamp(1.25rem,4vw,1.875rem)] text-text font-bold mb-6">{tTA('contact.title')}</h2>
                            <p className="md:text-lg text-[#335371]">{tTA('contact.description')}</p>
