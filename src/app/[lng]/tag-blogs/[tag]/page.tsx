...
import { useGetBlogsByTag, useGetLastBlogs } from "@/hooks/blog-hooks";
...
const tagBlogs = ({ params }: Props) => {
  const { tag } = params
  const { lng } = params
  const router = useRouter()
  const [searchTerm, setSearchTerm] = useState('')
  const { data, hasNextPage, fetchNextPage, error } = useGetBlogsByTag(6, decodeURI(tag), searchTerm)
    ...
    <>
      <div className='relative w-full h-fit -z-10 bg-center bg-no-repeat bg-cover bg-[url("../../../public/images/world-map.png")]'>
        ...
        <div className="container">
          <h1 className="pt-40 pb-28 text-center text-5xl font-BYekan">{t('tag')}:{decodeURI(tag)}</h1>
        </div>
      </div>
